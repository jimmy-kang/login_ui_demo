...
const LoginPage = (): JSX.Element => {
  ...
  const checkPasswordFormat = (val: string): void => {
    if ((val.length >= 8) !== isLengthFormat) {
      setLengthFormat(val.length >= 8)
    }
    ...
    <div className="slm-w-full slm-h-full slm-bg-blue-light slm-flex slm-items-center slm-justify-center">
      <LoginStyle className="slm-bg-white slm-rounded slm-p-6">
        ...
        <div className="login__content slm-m-auto">
          ...
          <div className="slm-text-3xl slm-mb-4 slm-font-bold font__normal">Create an account</div>
          <div className="slm-flex slm-justify-between slm-mb-4">
            ...
            <div className="slm-w-190px slm-h-12">
              <SignUpButton iconName={FaFacebookF} text="Sign up with Facebook"/>
            </div>
          </div>
          <div className="slm-text-sm slm-flex slm-items-center slm-mb-6">
            <span className="slm-mr-2 font__base">Or use your email for registration</span>
            <div className="slm-h-px slm-w-188px slm-bg-black"></div>
          </div>
          <form onSubmit={handleSubmit}>
            <div className="slm-flex slm-justify-between slm-mb-4">
              <div className="slm-w-190px slm-h-12"><FormInput title="First Name" name="firstName" /></div>
              <div className="slm-w-190px slm-h-12"><FormInput title="Last Name" name="lastName" /></div>
            </div>
            <div className="slm-w-full slm-h-12 slm-mb-4"><FormInput title="E-mail" name="email" type="email" /></div>
            ...
            <div className="slm-flex slm-flex-col slm-gap-6">
              <div className="slm-flex slm-gap-x-1">
                <div className="slm-flex slm-items-center">
                  <BsCheckCircleFill style={{ fontSize: '11px', color: isLengthFormat ? '#4AE7A5' : '#ABABAB' }} />
                  <p className={`slm-text-xs slm-scale-sm ${isLengthFormat ? 'black' : 'slm-text-gray-p1'}`}>8 Characters min.</p>
                </div>
                <div className="slm-flex slm-items-center">
                  <BsCheckCircleFill style={{ fontSize: '11px', color: isNumberFormat ? '#4AE7A5' : '#ABABAB' }} />
                  <p className={`slm-text-xs slm-scale-sm ${isNumberFormat ? 'black' : 'slm-text-gray-p1'}`}>One number</p>
                </div>
              </div>
              <div className="slm-grid slm-gap-y-6">
                ...
                />
                <button className="slm-w-full slm-h-12 slm-rounded slm-bg-blue-base slm-text-white slm-text-sm font__base" type="submit">Create an Free Account!</button>
              </div>
            </div>
